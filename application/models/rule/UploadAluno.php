...
class Model_Rule_UploadAluno extends Zend_File_Transfer_Adapter_Http {
    
    const DIR_IMAGE     = '/../public/images/fotos/';
    const DIR_THUMBNAIL = '/../public/images/fotos/thumbnail/';
    ...
    public function setNomeArquivo() {
        $this->nomeArquivo = md5(uniqid(time())) . '.'.$this->getExtension();
    ...
    public function upload() {
        ...
        return array('retorno' => 'sucesso', 'msg' => "Upload completo");
    ...
    public function criarImagem($filename) {
        try{
            ...
            if($this->getExtension() != 'gif' && $this->getExtension() != 'GIF') {
                ...
                $target_path = $newPath.basename($this->nomeArquivo);
                move_uploaded_file($_FILES['fileImage']['tmp_name'], $target_path);
            }


            $this->url = '/images/fotos/'.$this->nomeArquivo;
    ...
    public function criarThumbnail($filename) {
        try{
            ...
            $this->thumbnail = '/images/fotos/thumbnail/'.$this->nomeArquivo;
