...
class Utils_FrequenciaCalendario {
        ...
        public static function MostreCalendario( $mes, $aDias ){        
            $numero_dias = Utils_FrequenciaCalendario::GetNumeroDias( $mes );	// retorna o número de dias que tem o mês desejado
            $nome_mes = Utils_FrequenciaCalendario::GetNomeMes( $mes );
            $diacorrente = 0;	
	    $html = "";
            ...
            $html .= "<table border = '0' cellspacing = '0' align = 'center' style='display: inline; margin-right: 25px;'>";
            $html .= "<tr>";
            $html .= "<td colspan = 7 align='center' style='background: #0091d9; color: #fff;'><h3>".$nome_mes."</h3></td>";
            $html .= "</tr>";
            $html .= "<tr>";

            // função que mostra as semanas aqui
            $html .= Utils_FrequenciaCalendario::MostreSemanas();	
                    ...
                    if( $diacorrente + 1 <= $numero_dias ){
                        if( $coluna < $diasemana && $linha == 0){
                            ...
                            if(in_array($dia, $aDias)){
                                $html .= " align = 'center' style='background:yellow;' >";
                                $html .= "<a style='color: green;text-decoration:none;font-weight:bold;' href = ".$_SERVER["PHP_SELF"]."?mes=$mes&dia=".$dia.">".++$diacorrente . "</a>";
                            }else {
                                $html .= " align = 'center' >";
                                $html .= "<a style='color: #444;text-decoration:none;' href = ".$_SERVER["PHP_SELF"]."?mes=$mes&dia=".$dia.">".++$diacorrente . "</a>";
                            }
                        }
                    }else {
                        break;
        ...
        public static function MostreCalendarioCompleto(){
            ...
            for( $j = 0; $j < 4; $j++ ){
                ...
                for( $i = 0; $i < 3; $i++ ){
                    $html .= "<td>";
                    Utils_FrequenciaCalendario::MostreCalendario( ($cont < 10 ) ? "0".$cont : $cont );
